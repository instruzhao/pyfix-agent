...
The v0.3.1 benchmark runner uses `benchmarks/cases.yaml` schema v2. Tasks are generated by the runner and cannot contain case-specific hints. Agent-visible tests live inside each fixture; holdout tests live outside the fixture and run only after the repair loop. Reports include visible success, holdout success, final Success@1, Pass@k, regressions, failure types, prompt size, token usage, and per-run trace paths.
