...
Benchmark report schema 5 preserves the schema 4 repair, review, retrieval, indexing, and paired A/B measurements, then adds a secret-free protocol fingerprint and Wilson confidence intervals. `pyfixagent-benchmark-compare` joins only exact case, strategy, variant, and repetition identities; it fails closed when manifest, case selection, repetition count, strategy, or repository-mode metadata drift.
...
The default configuration uses Alibaba Cloud's `qwen3.7-plus` through the DashScope OpenAI-compatible endpoint. Thinking is enabled, and system instructions use the standard system role. `thinking_budget` remains absent by default; add it only after confirming the selected provider's supported request parameters.
...
`ContainerSandbox` is the default. `LocalSandbox` remains an explicit compatibility option for trusted projects; it runs a policy-checked subprocess on the host, disables bytecode writes, and enforces a timeout, but is not an isolation boundary.

`ContainerSandbox` starts one ephemeral Docker or Podman container per test command. It requires the repair engine's temporary Git worktree and mounts only that disposable workspace at `/workspace`. Its default policy disables networking and IPC sharing, makes the container root read-only, provides a bounded no-exec `/tmp`, drops all Linux capabilities, enables no-new-privileges, and limits CPU, memory, PIDs, open files, individual file size, combined output, sampled plus final worktree growth, and wall time. On POSIX hosts, `workspace_owner` resolves to the bind-mounted worktree owner's non-root UID/GID; root-owned workspaces are rejected instead of silently running the workload as root. For the default rootless Podman path, `--userns keep-id` preserves that host UID mapping for the bind mount. Docker Desktop falls back to the image's `65534:65534` user. Output is drained concurrently into a bounded host buffer. A timeout or policy violation force-removes the container by its generated name. The host repository, model credentials, and host environment are not passed into the container.

Dependencies follow an `image_only` policy: runtime installation commands are rejected and each runner profile is built from a digest-pinned base plus a fully resolved, wheel-hashed lock. The default scientific lock is joined by finite minimal and Flask web profiles. The recipe sets a non-root image user, OCI source/version labels, and an explicit profile label. Public CI generates provenance/SBOM attestations, scans application libraries with commit-pinned Trivy, builds all profiles, and exercises both Docker and Linux Podman. Version-tag publication attaches BuildKit SBOM/provenance plus GitHub OIDC build provenance to GHCR subjects. Trace metadata captures the requested image and best-effort resolved digest/ID. Operators who need other dependencies must build a reviewed image and select it in configuration or with `--container-image`.
