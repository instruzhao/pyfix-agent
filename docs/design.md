...
Dependencies follow an `image_only` policy: runtime installation commands are rejected and the runner image is built from a digest-pinned base plus a fully resolved, wheel-hashed `containers/requirements.lock`. The recipe sets a non-root image user and OCI source/version labels. Trace metadata captures the requested image and best-effort resolved digest/ID. Operators who need other dependencies must build a reviewed image and select it in configuration or with `--container-image`.
